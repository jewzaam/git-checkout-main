...
### VPN Management
- **SCOPE-001**: VPN connection management is explicitly out of scope
- **SCOPE-002**: Users must ensure required VPN connections are established before running GCM
- **SCOPE-003**: Network connectivity failures will be handled as standard git operation failures
...
- **REQ-020**: Support configuration file for user-specific settings
- **REQ-021**: Make all hardcoded values configurable:
  - Username mappings per provider
  - Remote names for forks
  - Custom branch naming patterns
- **REQ-022**: Support both global and per-repository configuration
